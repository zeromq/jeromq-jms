 ...
 */
@ZmqComponent("protobuf")
@ZmqUriParameter("event")
public class TestZmqProtobufEventHandler implements ZmqEventHandler {
    private static final Logger LOGGER = Logger.getLogger(TestZmqProtobufEventHandler.class.getCanonicalName());
        ...
        if (event instanceof ZmqSendEvent) {
            ...
            try {
                ...
                Message protoMessage = Message.newBuilder()
                        .setMessageId(messageId).setSendDate(currentTime).setBody(body).build();

                final byte[] data = protoMessage.toByteArray();
    ...
    public ZmqEvent createEvent(final ZmqSocketType socketType, final ZMsg msg) throws ZmqException {
        ...
        if (msgFrames.size() > 0) {
            ...
            try {
                ...
                final Message protoMessage = Message.parseFrom(msgFrameData);
                final ZmqTextMessage message = new ZmqTextMessage();

                message.setJMSMessageID(protoMessage.getMessageId());
                message.setJMSDeliveryTime(protoMessage.getSendDate());
                message.setText(protoMessage.getBody());

                final ZmqEvent sendEvent = new ProtobufEvent(message);

                return sendEvent;
            } catch (JMSException | InvalidProtocolBufferException ex) {
